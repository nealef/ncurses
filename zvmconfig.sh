...
OFLAGS="-O"
QFLAGS="-qlanglvl=extended:extc89:extc99 -qxplink -qenum=int -qexportall \
...
./configure --prefix=${PREFIX} CC="${CC}" CFLAGS="${CFLAGS}" CXX="${CXX}" \
	            CXXFLAGS="${CXXFLAGS}" LDFLAGS="${LDFLAGS}" CPP="${CPP}" \
		                CPPFLAGS="${CPPFLAGS}" --x-includes=${X_INCLUDES} \
					--disable-lp64 \
					--with-trace \
					--with-shared \
					--without-cxx-shared \
...
FILES="./c++/Makefile ./form/Makefile ./include/Makefile ./menu/Makefile ./ncurses/Makefile ./panel/Makefile ./progs/Makefile ./test/Makefile"
for f in ${FILES}
do
	sed -i'' -e '/^CPPFLAGS/s/= /= -qnosearch /' -e '/^CPPFLAGS/s+$+ -I/usr/include+' $f
done
exit 0
##############################################################################
